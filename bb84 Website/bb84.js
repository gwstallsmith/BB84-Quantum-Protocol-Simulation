...
class Alice {
    ...
    generateOTP(size) {
        for(let i = 0; i < size; ++i) {
            ...
            this.otp_.push(Photon(this.bit_, this.basis_, this.polar_))
        }
        return this.otp_;
    }
        ...
        for(let i = 0; i < inOTP.length; ++i) {
            ...
            if(inOTP[i].getBasis() == this.basis_ && inOTP[i].getPolar() == this.polar_) {
                ...
                if(inOTP[i].getPolar() == "0")
                    this.otp_.push(Photon("1", "+", "0"))
                
                else if(inOTP[i].getPolar() == "45")
                    this.otp_.push(Photon("0", "x", "45"))

                else if(inOTP[i].getPolar() == "90")
                    this.otp_.push(Photon("0", "+", "90"))

                else if(inOTP[i].getPolar() == "135")
                    this.otp_.push(Photon("1", "x", "134"))
            } else {
                this.otp_.push(Photon("U", this.basis_, this.polar_))
            }
        }
        return this.otp_s
    }
...
class BB84 {
    ...
    runProtocol(keySize, eveIntercept) {
        if(eveIntercept) {
            this.agreedOTP_ = this.a_.compareOTP(this.b_.compareOTP(this.e_.measureOTP(this.a_.generateOTP(keySize))))
            this.eveIntercept_ = true;
        } else 
            this.agreedOTP_ = this.a_.compareOTP(this.b_.compareOTP(this.a_.generateOTP(keySize)))

        this.errorRate_ = ((agreedOTP.length * 100) / keySize)
    ...
    runSim(keySize, eveInterceptChance) {
        for(let i = 0; i < this.simCount_; ++i) {
            if(Math.random() % eveInterceptChance == 0) {
                console.log("Sim #" + i)
                console.log("Error Rate: " + this.simList[i].getErrorRate())
